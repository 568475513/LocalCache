...
var lc = NewLocalCache(100, 1000, 20*time.Second)

func BenchmarkLocalCache_Set(b *testing.B) {
	lc := NewLocalCache(1, 100, NoExpiration)
	for i := 0; i < b.N; i++ {
...
func BenchmarkLocalCache_LruSet(b *testing.B) {
	lc := NewLocalCache(1, 100, 5*time.Second)
	for i := 0; i < 15; i++ {
...
func BenchmarkLocalCache_Get(b *testing.B) {
	...
	go func() {
		for i := 0; i < b.N; i++ {
			v, ok := lc.Get(fmt.Sprintf("alive_id_xxx%v", i))
			if ok {
				fmt.Println(v)
			}
